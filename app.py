...
def predict():
    ...
    clf1 = pkl.load(open('speech_classification1.pkl', 'rb'))
    clf2 = pkl.load(open('speech_classification2.pkl', 'rb'))
    clf3 = pkl.load(open('speech_classification3.pkl', 'rb'))
    clf4 = pkl.load(open('speech_classification4.pkl', 'rb'))
    clf5 = pkl.load(open('speech_classification5.pkl', 'rb'))
    clf6 = pkl.load(open('speech_classification6.pkl', 'rb'))
    ...
    fractions = {'normal': round((total[6]/total.sum())*100 ,2),'toxic': round((total[0]/total.sum())*100, 2),'severe_toxic': round((total[1]/total.sum())*100,2),'obscene':round((total[2]/total.sum())*100,2),'threat': round((total[3]/total.sum())*100,2),'insult': round((total[4]/total.sum())*100,2),'identity_hate' :round((total[5]/total.sum())*100,2)}

    
    #returning results with requested html page
    return render_template('result.html',fractions= fractions,plot_url=plot_data)
